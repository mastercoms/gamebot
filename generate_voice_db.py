...
# https://github.com/Jonarzz/DotaResponsesRedditBot/blob/ad61ed531c6de99461685f40f3dde8d400dcd1bb/parsers/wiki_parser.py
# Copyright (c) 2019 Jonarzz
# MIT License
...
def add_hero_and_responses(hero_name, response_link_list):
...
async def populate_responses():
    ...
    """
    await populate_hero_responses()
    await populate_chat_wheel()


async def populate_hero_responses():
    ...
    """
    pages = await pages_for_category(RESPONSES_CATEGORY)
    for page in pages:
        ...
        responses_source = (await wiki.get(url='/' + page, params={'action': 'raw'})).text

        response_link_list = await create_responses_text_and_link_list(responses_source=responses_source)
        ...
        add_hero_and_responses(hero_name=hero_name, response_link_list=response_link_list)


async def pages_for_category(category_name):
    ...
    json_response = (await wiki.get(url=API_PATH, params=params)).text

    pages = []

    parsed_json = orjson.loads(json_response)
...
async def create_responses_text_and_link_list(responses_source):
    ...
    * Add original response text, processed response text and file link to a list as a tuple.
    ...
    file_and_link_dict = await links_for_files(files_list)
...
def parse_response(text):
    ...
    if any(escape in text for escape in ['[[', ']]', '{{', '}}', '|', 'sm2']):
        print('Response could not be processed : ' + text)
        return None

    return text.strip()


async def links_for_files(files_list):
    ...
    files_link_mapping = {}
    futures = []
    empty_api_length = len(str(wiki.build_request("GET", url=API_PATH, params=get_params_for_files_api([])).url))
    ...
    for file in files_list:
        ...
        if file_name_len + current_title_length >= max_header_length - empty_api_length or \
            ...
            futures.append(wiki.get(API_PATH, params=get_params_for_files_api(files_batch_list)))
    ...
    for future in asyncio.as_completed(futures):
        ...
        for _, page in pages.items():
            ...
            try:
                ...
                print('File does not have a link : ' + title)

    return files_link_mapping


async def populate_chat_wheel():
    ...
    """
    chat_wheel_source = (await wiki.get('/' + 'Chat_Wheel', params={'action': 'raw'})).text
    ...
    for match in chat_wheel_regex.finditer(chat_wheel_source):
        ...
        add_hero_and_responses(hero_name=event, response_link_list=response_link_list)

wiki : httpx.AsyncClient = None

async def main():
    global wiki
    transport = httpx.AsyncHTTPTransport(retries=5)
    async with httpx.AsyncClient(base_url=URL_DOMAIN, timeout=10.0, http2=True, transport=transport) as wiki:
